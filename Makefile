...
uv-sync-lint: prerequisites
	@echo -e "\u23F3 Syncing linting dependencies..."
	@uv sync --group types --group lint --dev -q >/dev/null 2>&1
	@echo -e "\u2705 Linting dependencies synced."
...
exe: lint
	...
	@uv sync --no-dev --no-default-groups --reinstall -q >/dev/null 2>&1
	@uvx -p 3.10 --from pip-tools pip-compile ./pyproject.toml --output-file requirements.txt --no-header --no-annotate --strip-extras -q >/dev/null 2>&1
	@uvx --with-requirements ./requirements.txt -p 3.10 pyinstaller \
		...
		--add-data "src/leaguewizard/data/cert:leaguewizard/data/cert" \
		--log-level ERROR \
		--exclude "pydantic.experimental" \
		--exclude "pydantic.experimental.arguments_schema" \
		--exclude "pydantic.experimental.pipeline" \
